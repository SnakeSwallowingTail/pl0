...
/*当函数中会发生fatal error时，返回-1，告诉调用它的函数，最终退出程序*/
...
int getch(void);
void init(void);
int gen(enum fct x,int y,int n);
