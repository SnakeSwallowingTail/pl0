...
int main()
{
	...
	/*scanf("%s",fname);*/      /*多少有点危险的操作*/
	scanf("%9s",fname);         /*建议限制输入长度*/
	fin=fopen(fname,"r");
	if(fin)
	{
 ...
 * 漏掉空格，读取一个字符
 *
 * 每次读取一行，存入line缓冲区，line被getsym取空后再读取一行
 *
 * 被函数getsym调用。
 */
int getch()
 ...
 * （该部分的后跟符号），test负责这项检测，并且负责当检测不通过是的补救措施
...
int block(int lev,int tx,bool* fsys)
{
	...
	table[tx0].adr=cx;             /* 声明部分中每增加一条声明都会给dx增加1,声明部分
	...
	gendo(opr,0,0);                          /* 每个过程出口都要使用释放数据段指令*/
 ...
 * ptx:名字表尾指针的指针，为了可以公平名字表尾指针的值
 * lev:名字所在层次，以后所有的lev都是这样
			...
			if(sym==writesym)  /*准备按照write语句处理，与read类似*/
			{
				...
				if(sym==lparen)
				{
					do
					{
						...
						memcpy(nxtlev,fsys,sizeof(bool)*symnum);
						nxtlev[rparen]=true;
						nxtlev[comma]=true;  /*write后跟符号为) or ,*/
...
int factor(bool* fsys,int* ptx,int lev)
{
	int i;
	bool nxtlev[symnum];
	testdo(facbegsys,fsys,24);                     /* 检测因子开始的符号*/
	...
	do
	{
		...
		switch(i.f)
		{
			...
			case opr:               /* 数学、逻辑运算*/
				...
				break;
			case lod:               /* 取相对当前过程的数据基地址为a的内存到栈顶*/
				s[t]=s[base(i.l,s,b)+i.a];
				t++;
				break;
			case sto:               /* 栈顶的值保存到相对当前过程的数据基地址为a的内存*/
				t--;
